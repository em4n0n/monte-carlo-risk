...
if st.button("Run Simulation"):
    ...
    # Get the screen width from Streamlit (in pixels)
    screen_width = st.get_option("browser.window_width")

    # Set the figure size dynamically
    fig_width = screen_width // 100  # This divides screen width by 100 to get an appropriate size
    fig_height = fig_width * 0.6  # Adjusting the height to be proportional to the width

    # Plot ending balance distribution
    fig1, ax1 = plt.subplots(figsize=(fig_width, fig_height))  # Use dynamic size here
    ...
    # Filter PnL curves (showing first 50 successful or busted runs based on the selected filter)
    indices = np.where(np.array(ending_balances) >= 53000)[0]  # Successful runs
    fig2, ax2 = plt.subplots(figsize=(fig_width, fig_height))  # Use dynamic size here
    for i in indices[:50]:
        ax2.plot(ending_balances[i], alpha=0.3)  # Sample plot for successful runs
    ax2.set_title(f"Sample PnL Curves - Successful Runs")
