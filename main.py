...
def main():
    ...
    starting_capital = st.number_input(
        "Starting Capital ($)", min_value=1000.0, max_value=1_000_000.0,
    ...
    max_daily_loss  = st.number_input(
        ...
        max_value=starting_capital, value=1000.0, step=100.0
    )
    bust_threshold  = st.number_input(
        "Bust Threshold ($)", min_value=0.0,
        max_value=starting_capital, value=0.0, step=100.0
    )
    ...
    target_balance  = st.number_input(
        "Success Target ($)", min_value=starting_capital,
        max_value=starting_capital*2, value=starting_capital+3000.0, step=100.0
    )
    ...
    if st.button("Run Simulation"):
        ...
        st.write(f"Average Max Drawdown:       {np.mean(max_dds):.2%}")
        ...
        # Equity Curve with Probability Cone (fixed ragged-array issue)
        curves_list = [s['curve'] for s in sims]
        max_len     = max(len(c) for c in curves_list)
        padded      = np.array([
            np.pad(c, (0, max_len - len(c)), 'edge') for c in curves_list
        ])
